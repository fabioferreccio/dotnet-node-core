...
export class AnsiEncoder {
    ...
    public static GetStyle(style: string): string {
        switch (style.toLowerCase()) {
            case "bold": return `${AnsiEncoder.ESC}[1m`;
            case "dim": return `${AnsiEncoder.ESC}[2m`;
            case "italic": return `${AnsiEncoder.ESC}[3m`;
            case "underline": return `${AnsiEncoder.ESC}[4m`;
            case "invert": return `${AnsiEncoder.ESC}[7m`;
            default: return "";
        }
    }
}
